...
func (s *Submitter) Submit(ctx context.Context, form *portal.FormData, creds *credential.Credentials) error {
	...
	if len(respBody) > 0 {
		...
		// Check for common captive portal auth failure signatures in response body
		lowerSnip := strings.ToLower(snip)
		authErrorKeywords := []string{
			"invalid username", "invalid password", "authentication failed",
			"login failed", "user name or password error", "password error",
			"user or password error", "incorrect username", "incorrect password",
		}
		for _, kw := range authErrorKeywords {
			if strings.Contains(lowerSnip, kw) {
				s.logger.Warn("portal response indicates potential authentication failure", "keyword", kw)
				break
			}
		}
	}
